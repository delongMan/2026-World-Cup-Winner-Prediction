...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="bg-[#060b14] text-white min-w-max">
      <header className="flex items-center justify-between py-2 sm:py-3 bg-[#060b14] fixed top-0 left-0 right-0 z-50 gap-1 sm:gap-2" style={{ paddingLeft: 'max(0.75rem, var(--safe-left))', paddingRight: 'max(0.75rem, var(--safe-right))', paddingTop: 'max(0.5rem, var(--safe-top))' }}>
      ...
      <main id="prediction-content" className="pt-11 sm:pt-14">{children}</main>
    </div>
  );
}
