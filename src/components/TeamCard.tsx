...
export function TeamCard({ team, matchId, isWinner, isLoser, canInteract, canDrag, onClick }: P) {
  ...
  const style = transform
    ? { transform: `translate(${transform.x}px, ${transform.y}px)`, zIndex: 60, position: 'relative' as const }
    : undefined;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-2.5 px-1.5">
        <p className={`text-[11px] font-bold leading-tight text-center ${isWinner ? 'text-white' : 'text-white/85'} drop-shadow-[0_2px_6px_rgba(0,0,0,0.8)]`}>{team.nameZh}</p>
      </div>
      {isWinner && (
